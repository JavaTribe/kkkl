...
var Layout = function() {
	...
	var handleSidebarMenu = function() {
		...
		// myth.com-ylm:ajax to page method
		jQuery(document).on('click', '.ajax-to', function(e) {
			e.preventDefault();
			Metronic.scrollTop();
		...
		jQuery('.page-content').on('click', '.ajaxify', function(e) {
			...
			$.ajax({
				...
				success : function(res) {
					...
					//不在操作时间段内出现modal
					if(res.message == "process_not_start") {
						var $modal = $('.ajaxifyNotStartModal');
						if($modal.length == 0) {
							$('<div class="modal fade ajaxifyNotStartModal" tabindex="-1" data-keyboard="false" data-width="500"><div class="modal-body"><p>'+res.process+'未开始</p></div><div class="modal-footer"><a class="btn blue" data-dismiss="modal">确定</a></div></div>')
							.appendTo($('body'));
						...
						$modal.modal();
						return;
					} else if(res.message == "process_end") {
						var $modal = $('.ajaxifyProcessEndModal');
						if($modal.length == 0) {
							$('<div class="modal fade ajaxifyProcessEndModal" tabindex="-1" data-keyboard="false" data-width="500"><div class="modal-body"><p>'+res.process+'已结束</p></div><div class="modal-footer"><a class="btn blue" data-dismiss="modal">确定</a></div></div>')
							.appendTo($('body'));
						...
						$modal.modal();
						return;
					} else if(res.message == "process_disable") {
						var $modal = $('.ajaxifyProcessDisableModal');
						if($modal.length == 0) {
							$('<div class="modal fade ajaxifyProcessDisableModal" tabindex="-1" data-keyboard="false" data-width="500"><div class="modal-body"><p>'+res.process+'不可用</p></div><div class="modal-footer"><a class="btn blue" data-dismiss="modal">确定</a></div></div>')
							.appendTo($('body'));
	...
	return {
		...
		ajaxSubmit : function(formId, submitId) {
			...
			$.ajax({
				...
				success : function(res) {
					...
					if(res.message == "delay") {
						...
						$modal.load('common/delayreason/toDelayReasonForm', '', function(){
							$modal.modal();
						});
						return;
					//超时不给补交咯
					} else if(res.message == "process_not_start") {
						var $modal = $('.ajaxifyNotStartModal');
						if($modal.length == 0) {
							$('<div class="modal fade ajaxifyNotStartModal" tabindex="-1" data-keyboard="false" data-width="500"><div class="modal-body"><p>'+res.process+'未开始</p></div><div class="modal-footer"><a class="btn blue" data-dismiss="modal">确定</a></div></div>')
							.appendTo($('body'));
